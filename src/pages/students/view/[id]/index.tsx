...
function StudentViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<StudentInterface>(
    ...
    () =>
      getStudentById(id, {
        relations: ['parent_student_parent_idToparent', 'organization', 'parent_parent_student_idTostudent'],
      }),
  );
            ...
            <Box>
              {hasAccess('parent', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    Parent Student Parent Id Toparent:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/parents/view/${data?.parent_student_parent_idToparent?.id}`}>
                      {data?.parent_student_parent_idToparent?.login_details}
                    </Link>
                  </Text>
                </Flex>
              )}
                ...
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Parent Parent Student Id Tostudents
                    </Text>
                    <NextLink passHref href={`/parents/create?student_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.parent_parent_student_idTostudent?.map((record) => (
                          <Tr
                            cursor="pointer"
